...
try:
    ...
    # Support running from the package directory or as a module
...
def app() -> None:
    ...
    if run_btn:
        try:
            if use_default:
                # Prefer the example file bundled next to this script
                excel_path = Path(__file__).parent / "Trading Case Example Data.xlsx"
                if not excel_path.exists():
                    # Fall back to repo root for backward compatibility
                    excel_path = Path("Trading Case Example Data.xlsx")
                if not excel_path.exists():
                    st.error("Could not find 'Trading Case Example Data.xlsx' in the package or project root.")
                    st.stop()
            else:
                if uploaded is None:
...
# Run the app at import-time so Streamlit shows the UI when importing the script
app()
